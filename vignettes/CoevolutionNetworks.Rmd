...
While the previous steps have only utilized a small subsample of the data, we've
pre-computed the complete dataset for you to play with. This dataset is comprised
...
Let's start by loading in our input data. For people downloading the datafiles,
these files are found in `CoevNetworks/`.
...
highConf <- sapply(CogsAnnot, \(x) 
                   if(is(x, 'Taxa')) 
                     max(sapply(x, \(y) y$confidence[length(y$confidence)])) > 50
                   else FALSE)
