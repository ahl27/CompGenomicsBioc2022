...
```{r}
library(SynExtend)
datafile <- system.file('extdata', 'treesFromCOGs.RData', mustWork=TRUE)
...
Jaccard <- predict(pw, method='Jaccard') # Jaccard distance of P/A
Hamming <- predict(pw, method='Hamming') # Hamming distance of P/A
MutualInf <- predict(pw, method='MutualInformation') # MutualInformation of P/A
ProfDCA <- predict(pw, method='ProfDCA') # Direct Coupling Analysis of P/A
...
MirrorTree <- predict(pw, method='MirrorTree') # MirrorTree analysis
ContextTree <- predict(pw, method='ContextTree') # ContextTree analysis

# Residue Methods: (ONLY AVAILABLE IN DEV VERSION)
#ResidueMI <- predict(pw, method='ResidueMI') # MutualInf of residues
#DendroMI <- predict(pw, method='DendroMI') # MutualInf of changes on phylogeny
```
