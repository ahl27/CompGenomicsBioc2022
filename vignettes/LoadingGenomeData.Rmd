...
set of 17 complete *Micrococcus* genomes obtained from [NCBI GenBank](https://www.ncbi.nlm.nih.gov/data-hub/genome/?taxon=1269&annotated_only=true&refseq_annotation=true&assembly_level=3%3A3). I've predownloaded these and repackaged them into 
a single `.FASTA` file for this workshop (the file extension is `.fa.gz` since it's been compressed for size).

These sequences have been trimmed to facilitate completing analysis in a single 
workshop slot. Feel free to experiment with larger sequences later.

If you can't find the FASTA file, you can download it on the [Setup page](https://www.ahl27.com/CompGenomicsBioc2022/articles/Setup.html). You can
also build this object yourself by downloading the `_genomic.fasta` files
...
```{r, eval=FALSE}
# This datafile is available from materials on the Setup page
# It contains the first 2.5KB from all complete Micrococcus genomes on Genbank
# We'll work with the complete sequences later in the workshop
exampledata <- load('LoadingData/MicroFASTAtrimmed.fa.gz')
...
and is significantly more accurate for coding sequences.
