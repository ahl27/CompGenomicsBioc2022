...
phylogenies without rooting them. You can plot them just as well with `plot`, I
just prefer visualize unrooted trees without an arbitrary root.

```{r}
library(DECIPHER)
library(ape)
datafile <- system.file('extdata', 'phyloInputData.RData', 
                        package='CompGenomicsBioc2022', mustWork=TRUE)
...
treeMP <- TreeLine(testSeqs, method='MP')
plot_tree_unrooted(treeMP, title='MP')
treeNJ <- TreeLine(testSeqs, method='NJ')
plot_tree_unrooted(treeNJ, title='NJ')
treeUltra <- TreeLine(testSeqs, method='complete')
plot(treeUltra, title='Ultrametric')
```
...
# Max runtime is a minute
# Note that method='ML' is the default setting
treeML <- TreeLine(testSeqs, maxTime=60)
plot_tree_unrooted(treeML, title='Maximum-Likelihood')
```
...
specTree <- TreeLine(testSeqs, model=mySpecificModel, maxTime=60)
plot_tree_unrooted(specTree, title='Specific Set ML')
...
exclTree <- TreeLine(testSeqs, model=myExcludedModel, maxTime=60)
plot_tree_unrooted(exclTree, title='Excluded Set ML')
```
