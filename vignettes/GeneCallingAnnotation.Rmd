...
For this example, we'll be using a complete Archaeal genome taken from NCBI GenBank.
...
geneticRegions <- FindNonCoding(ncRNA, archaealGenome)
...
# Find Genes
genes <- FindGenes(archaealGenome, includeGenes=geneticRegions)
```
...
# Find sequences corresponding to found genes
geneSeqs <- ExtractGenes(genes, archaealGenome, type="DNAStringSet")
geneSeqs
```
...
# Load Training File
# This is an IDTAXA training data file available from www2.decipher.codes/Downloads.html
# This contains a training set called 'trainingSet' generated from Firmcutes KEGG data
trainingFile <- system.file('extdata', 'KEGG_Firmicutes_r95.RData',
                            package='CompGenomicsBioc2022')
load(trainingFile) 
...
# Classify!
ids <- IdTaxa(cogSeqs, trainingSet)
```
